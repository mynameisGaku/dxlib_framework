#!/usr/bin/env python3
"""Install the pinned TaskDispatcher recovery patch. Dry-run unless --apply is supplied."""
...
TARGETS = frozenset({
    'Source/Runtime/Public/Dxf/TaskDispatcher.h',
    'Source/Runtime/Private/Dxf/TaskDispatcher.cpp',
    'Tests/TaskDispatcherRecoveryTests.cpp',
    'Tools/TaskValidation/CMakeLists.txt',
    'Tools/TaskValidation/Main.cpp',
    'Docs/TaskDispatcherRecovery.md',
})
...
def safe_path(root: Path, relative: str) -> Path:
    ...
    if not relative or '\\' in relative or rel.is_absolute() or any(p in ('..', '.') for p in rel.parts):
...
def build_plan(root: Path, package: Path = HERE) -> list[Change]:
    ...
    if manifest['format'] != 1 or set(entries) != TARGETS:
    ...
    for rel in sorted(TARGETS):
        metadata = entries[rel]
        payload = safe_path(package / 'Payload', rel)
        ...
        if before is not None and normalized(before) == normalized(data):
            # Exact installed result: do not touch it, even when it is not committed yet.
            continue
        expected = metadata['before_git_blob_sha1']
        if expected is None:
            if before is not None:
                raise ValueError(f'New path already contains different content: {rel}')
        elif before is None or blob_digest(before) != expected:
            raise ValueError(f'Target differs from the inspected pre-patch source: {rel}')
        dirty = has_changes(root, rel, before is not None)
        if dirty:
...
def atomic_write(path: Path, data: bytes, mode: int) -> None:
    ...
    descriptor, temporary = tempfile.mkstemp(prefix='.dxf-task-', dir=path.parent)
    temp = Path(temporary)
    try:
...
def apply_plan(root: Path, plan: list[Change]) -> Path | None:
    ...
    backup = Path(tempfile.mkdtemp(prefix=root.name + '.task-backup-', dir=root.parent))
    backup_records = []
    for change in plan:
