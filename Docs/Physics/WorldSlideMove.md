...
初期接触からの離脱・押し出し、接地・ジャンプ・段差・坂の制御、複数の面を使う反復、動く床への追従は行いません。完成したキャラクターコントローラーや、どんな状況でも動き続けられる移動機能ではありません。それらは別の入口の[キャラクター移動（MoveAndSlide・StepCharacter・移動Component）](CharacterMovement.md)で扱います（この関数の契約は変えていません）。
