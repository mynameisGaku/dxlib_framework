...
- 候補は、Worldが自動で保つ索引（[World問い合わせの索引](QueryAcceleration.md)）で経路全体（半径を含む）の範囲に重なるColliderだけに絞ります。木の訪問はおおむね対数で、判定の数は経路の近くの候補数に比例します（長い経路・密集では増え、索引を使えない条件では保持スロット数nに対しO(n)の総当たり）。結果・順序・失敗は総当たりと同じです。追加領域O(1)です。通常経路で配列を確保しません（ヒット・非交差・静止・マスク0・半径0、登録・移動の直後で確保が起きないことを隔離した試験で確認）。測定は[検証記録](../Development/QueryScale-2026-09-25.md#性能測定)を参照してください。
