...
RequestChangeは要求の受付だけです。通常の連続した複数要求は最後の要求を採用します。ただし、Pending Sceneの差し替え時に発生するデストラクタからの再入Commit／差し替えは拒否します。次の境界で新Sceneを初期化し、成功した場合だけ旧Sceneを終了して差し替えます。失敗時は準備中のSceneだけを片付け、現在のSceneを維持します。
...
途中で終了が要求された場合、子階層へ停止予約を伝え、実行中のユーザーコールバックが返るまで破棄しません。残りの初期化・更新・描画・提示を抑止します。停止予約はフラグ変更だけで、終了フックやメモリ解放を直接行いません。
