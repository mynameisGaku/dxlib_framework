...
FRenderSystem::FRenderSystem(IRenderBackend& Backend)
...
// @param Height 高さ。
// @param Color 描画色。
TResult<void> FRenderSystem::BeginFrame(Toolbox::int32 Width, Toolbox::int32 Height, FColor Color)
{
	if (!m_Context.IsOwnerOperationAllowed_Internal() || m_bBusy || m_bFrame)
	{
...
// 退避した描画先を復元する。
TResult<void> FRenderSystem::RestoreTarget_Internal()
{
	// テクスチャを描画先に設定しているか。
...
// @param Error エラー情報。
TResult<void> FRenderSystem::FailFrame_Internal(const FError& Error)
{
	...
	m_Context.Clear3D_Internal();
	m_Queue.Clear_Internal();
	return TResult<void>::Failure(*m_FrameError);
}
// 蓄積した描画命令を実行する。
TResult<void> FRenderSystem::Flush_Internal()
{
	if (m_FrameError)
	{
		return TResult<void>::Failure(*m_FrameError);
...
// 蓄積した描画命令を実行する。
TResult<void> FRenderSystem::Flush()
{
...
TResult<void> FRenderSystem::SetRenderTarget(const FRenderTarget& Target)
{
	if (!m_Context.IsOwnerOperationAllowed_Internal() || !m_bFrame || m_bBusy)
	{
		return StateError_Internal();
...
// 画面のバックバッファを設定する。
TResult<void> FRenderSystem::SetBackBuffer()
{
	if (!m_Context.IsOwnerOperationAllowed_Internal() || !m_bFrame || m_bBusy)
	{
		return StateError_Internal();
...
// 現在の描画先を指定色で消去する。
// @param Color 描画色。
TResult<void> FRenderSystem::ClearTarget(FColor Color)
{
	if (!m_Context.IsOwnerOperationAllowed_Internal() || !m_bFrame || m_bBusy)
	{
		return StateError_Internal();
...
TResult<void> FRenderSystem::Native(const Toolbox::TFunction<TResult<void>()>& Callback)
{
	if (!m_Context.IsOwnerOperationAllowed_Internal() || !m_bFrame || m_bBusy || !Callback)
	{
		return StateError_Internal();
	}
	// 処理終了時に状態を戻すガード。
...
TResult<void> FRenderSystem::EndFrame()
{
	if (!m_Context.IsOwnerOperationAllowed_Internal() || !m_bFrame || m_bBusy)
	{
		return StateError_Internal();
...
void FRenderSystem::CancelFrame() noexcept
{
	if (!m_Context.IsOwnerOperationAllowed_Internal() || m_bBusy)
	{
		return;
	}
	m_Context.Clear3D_Internal();
