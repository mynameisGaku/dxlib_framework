...
public:
    ...
    int GetWidth() const noexcept { return m_pResource ? m_pResource->GetMetadata().Width : 0; }
    int GetHeight() const noexcept { return m_pResource ? m_pResource->GetMetadata().Height : 0; }
    int GetNativeHandle_Internal() const noexcept { return m_pResource ? m_pResource->GetHandle_Internal() : -1; }
    const std::shared_ptr<FTextureResource>& GetResource_Internal() const noexcept { return m_pResource; }
private:
...
public:
    ...
    const FTexture& AsTexture() const noexcept { return m_Texture; }
