...
public:
    ...
    {
        if (m_bInitialized) { return TResult<void>::Failure(EErrorCode::InvalidState, "Session already initialized"); }
        if (Settings.Width <= 0 || Settings.Height <= 0) { return TResult<void>::Failure(EErrorCode::InvalidArgument, "Invalid window size"); }
        auto Result = m_pPlatform->Initialize(Settings);
        m_bInitialized = static_cast<bool>(Result); return Result;
    }
    void Shutdown() noexcept
    {
        if (m_bInitialized) { m_bInitialized = false; m_pPlatform->Shutdown(); }
    }
