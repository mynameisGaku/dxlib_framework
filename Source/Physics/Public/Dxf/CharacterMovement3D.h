...
#ifndef DXF_CHARACTER_MOVEMENT_3D_H
#define DXF_CHARACTER_MOVEMENT_3D_H
#include "Dxf/CharacterGround3D.h"
#include "Dxf/CharacterMoveInput3D.h"
...
/**
 * 球をDisplacementだけ動かす候補を、接触面に沿って反復的に解決する（Worldは変更しない）。
 * 開始時に接触余裕以内の面と、移動中に当たった面を制約にし、残りの移動からすべての制約の内向き成分を除いて進む
 * （3Dは二つの面の稜線に沿い、三つの面に囲まれると止まる）。当たった面へは接触余裕の距離まで近づく。
