...
#ifndef DXF_CHARACTER_MOVE_RESULT_2D_H
#define DXF_CHARACTER_MOVE_RESULT_2D_H
 ...
 * 移動中に制約として使った接触面。
 */
struct FCharacterMoveContact2D
{
	/**
	...
	/**
	 * 対象から円の中心へ向く単位法線（制約として使った値）。
	 */
	Toolbox::FVector2 Normal;
};
/**
 * MoveAndSlideの結果。Worldや内部配列を参照しない値。
 */
struct FCharacterMoveResult2D
{
	/**
	 * 保持できる接触面の最大数。
	 */
	 ...
	 */
	Toolbox::FVector2 Applied;
	/**
	 * 処理しなかった移動（上限・方向不明などで止まった場合）。制約で取り除いた内向き成分は含まない。
	 */
	Toolbox::FVector2 Remaining;
	/**
	 * 止まった理由。
	 */
	 ...
	 */
	Toolbox::TArray<FCharacterMoveContact2D, MaxContacts> Contacts;
	/**
	 * Contactsのうち有効な件数。
	 */
