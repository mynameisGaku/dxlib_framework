...
#include "Dxf/WorldContactSet2D.h"
...
public:
	 ...
	 * 結果のNormalは接触対象から問い合わせ円の中心へ向く単位方向。初期接触・半径0・丸めで方向を区別できない場合は空（ヒットは成立）。
	...
	Toolbox::TVector<FColliderId2D> OverlapAll(const Toolbox::FCircle2D& Area,
	                                           ...
	                                           const FWorldQueryFilter& Filter = {}) const;
	/**
	 * 問い合わせ円と現在の各Colliderの符号付き距離を求め、距離がMargin以下（接触・重なりを含む）のものを返す。
	 * 分離方向はToolbox::FindShapeContactと同じ規則（回転矩形はSweepClosestと同じ角度規約）。結果は固定容量で配列を確保しない。
	 ...
	 * 問い合わせでStep・起床・採取・力の消去を行わない。変更・Stepと外側で直列化する。
	 * @param Shape 調べる円（半径0は点）。物理ワールド座標。
	 * @param Margin 結果に含める最大の符号付き距離（有限・非負。0で接触・重なりだけ）。
	 * @param ExcludedBody 任意の自己Body。除外しない場合は空Optional。
	 * @param Filter 対象にする問い合わせカテゴリ。既定は全ビット。
	 */
	FWorldContactSet2D QueryContacts(const Toolbox::FCircle2D& Shape, Toolbox::f64 Margin,
	                                 Toolbox::TOptional<FBodyId2D> ExcludedBody = {},
	                                 const FWorldQueryFilter& Filter = {}) const;
	/**
	 ...
	 * @param StartShape 開始時の円（中心と正の半径）。物理ワールド座標。
	 * @param EndCenter 終点の中心。変位・速度ではない。
	 * @param ExcludedBody 任意の自己Body。除外しない場合は空Optional。
	 * @param Filter 対象にする問い合わせカテゴリ。既定は全ビット。
	 */
	Toolbox::TOptional<FWorldSweepHit2D> SweepClosestIgnoringInitialContacts(
	    const Toolbox::FCircle2D& StartShape, Toolbox::FVector2 EndCenter,
	    Toolbox::TOptional<FBodyId2D> ExcludedBody = {}, const FWorldQueryFilter& Filter = {}) const;
...
private:
	 ...
	 */
	Toolbox::TOptional<FWorldSweepHit2D> SweepColliders_Internal(const Toolbox::FCircle2D& StartShape,
