...
#ifndef DXF_PHYSICS_DEBUG_SNAPSHOT_2D_H
 ...
 */
inline constexpr Toolbox::size_t MaxPhysicsDebugColliders2D = 256;
/**
 * 1回の観察でWorldから複製するBodyの上限。Colliderを持たないBodyも数える。
 */
inline constexpr Toolbox::size_t MaxPhysicsDebugBodies2D = 4096;
/**
 * 一つのColliderを表示用のワールド座標へ変換した値。Native資源やWorldへの参照を持たない。
 */
struct FPhysicsDebugItem2D
{
	/**
	 ...
	 * 採取時のBody重心位置。メートル単位。
	 */
	Toolbox::FVector2 CenterOfMass;
	/**
	 * 採取時のBody姿勢角。ラジアン単位で反時計回りが正。
	 */
	Toolbox::f32 BodyAngle = 0;
	/**
	 * 採取時のBody重心速度。メートル毎秒。
	 */
...
/**
 * Worldが採取したFPhysicsSnapshot2Dを表示用に変換した値。接触・Impulseの再計算は行わない。
 */
struct FPhysicsDebugSnapshot2D
{
	/**
	 * 採取元World。0は未採取を表す。
	 */
	 ...
	 */
	Toolbox::TVector<FPhysicsDebugItem2D> Items;
};
/**
 ...
 * @param Source FPhysicsWorld2D::CaptureSnapshotの戻り値。
