...
namespace Dxf
{
enum class ELifecycleState { Pending, Initializing, Active, Stopping, Stopped };
/** Framework dispatch is fixed; user overrides only the On* hooks. */
class DLifecycleObject : public DObject
{
public:
    ELifecycleState GetState() const noexcept { return m_State; }
    bool IsInitialized() const noexcept { return m_State == ELifecycleState::Active; }
    bool IsDestroyRequested() const noexcept { return m_bDestroyRequested; }
    void SetUpdateOrder(int Order) noexcept { m_UpdateOrder = Order; }
    int GetUpdateOrder() const noexcept { return m_UpdateOrder; }
    void SetTickWhenPaused(bool bEnabled) noexcept { m_bTickWhenPaused = bEnabled; }
    ...
    void RequestDestroy_Internal() noexcept { m_bDestroyRequested = true; }
    void SetCreationOrder_Internal(std::uint64_t Order) noexcept { m_CreationOrder = Order; }
    std::uint64_t GetCreationOrder_Internal() const noexcept { return m_CreationOrder; }
    ILifecycleGroup* GetChildren_Internal() noexcept { return m_pChildren; }
protected:
    DLifecycleObject() = default;
    virtual TResult<void> OnInitialize(const FInitContext&) { return {}; }
    ...
    void AttachChildren_Internal(ILifecycleGroup& Children) noexcept { m_pChildren = &Children; }
private:
