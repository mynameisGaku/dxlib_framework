...
public:
    ...
    ~TManagedCollection() override { Shutdown_Internal(); }
    ...
    TResult<TObjectHandle<U>> Spawn(TArgs&&... Args)
    {
        ...
        catch (const std::exception& Error) { return TResult<TObjectHandle<U>>::Failure(EErrorCode::UserException, Error.what()); }
        catch (...) { return TResult<TObjectHandle<U>>::Failure(EErrorCode::UserException, "Unknown constructor exception"); }
    }
    template <typename U> bool Destroy(const TObjectHandle<U>& Handle) noexcept
    {
        T* Object = m_Storage.Find_Internal(Handle.GetId());
        if (!Object || Object->IsDestroyRequested()) { return false; }
        Object->RequestDestroy_Internal(); return true;
    }
    std::size_t Size() const noexcept { return m_Storage.Size(); }
    void FreezeBoundary_Internal() override
    { if (!m_bBusy && m_bAccepting) { m_Lifecycle.FreezeBoundary_Internal(); } }
    TResult<void> CommitBoundary_Internal(const FInitContext& Context) override
    {
        if (m_bBusy) { return BusyError_Internal(); }
        if (!m_bAccepting) { return {}; }
        TResult<void> Result;
        { TGuardValue Guard(m_bBusy, true); Result = m_Lifecycle.CommitBoundary_Internal(Context); }
    ...
    TResult<void> Tick_Internal(const FTickContext& Context) override
    {
        ...
        if (!m_bAccepting) { return {}; }
        TResult<void> Result;
        { TGuardValue Guard(m_bBusy, true); Result = m_Updater.Tick_Internal(Context); }
        FinishDispatch_Internal(); return Result;
    }
    TResult<void> Draw_Internal(FRenderContext& Context) override
    {
        ...
        if (!m_bAccepting) { return {}; }
        TResult<void> Result;
        { TGuardValue Guard(m_bBusy, true); Result = m_DrawDispatcher.Draw_Internal(Context); }
        FinishDispatch_Internal(); return Result;
    }
    void Shutdown_Internal() noexcept override
    {
        if (m_bBusy) { m_bShutdownRequested = true; return; }
        if (!m_bAccepting) { return; }
        m_bAccepting = false; m_bShutdownRequested = false;
        TGuardValue Guard(m_bBusy, true); m_Lifecycle.Shutdown_Internal();
    }
protected:
    virtual void PrepareObject_Internal(T&) {}
private:
    static TResult<void> BusyError_Internal() { return TResult<void>::Failure(EErrorCode::InvalidState, "Reentrant collection dispatch"); }
    void FinishDispatch_Internal() noexcept { if (m_bShutdownRequested) { Shutdown_Internal(); } }
