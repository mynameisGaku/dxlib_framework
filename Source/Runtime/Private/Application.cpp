...
TResult<void> FApplication::Start_Internal(std::unique_ptr<DScene> InitialScene)
{
    ...
    if (!Commit) { return TResult<void>::Failure(Commit.Error()); }
    m_bStarted = true;
    return {};
}
TResult<void> FApplication::Start(std::unique_ptr<DScene> InitialScene)
{
    ...
    if (!Result || WantsQuit_Internal()) { Shutdown(); }
    return Result;
}
...
{
    if (WantsQuit_Internal()) { return TResult<bool>::Success(false); }
    auto Events = m_pPlatform->PumpEvents();
    if (!Events) { return Events; }
    if (!Events.Value()) { return TResult<bool>::Success(false); }
    auto Time = m_Clock.Sample(NowSeconds);
    if (!Time) { return TResult<bool>::Failure(Time.Error()); }
    auto Input = m_Input.Update();
    if (!Input) { return TResult<bool>::Failure(Input.Error()); }
    auto Transition = m_Scenes.Commit();
    // A failed replacement is recoverable while the previous scene remains active.
    if (!Transition && !m_Scenes.GetCurrent()) { return TResult<bool>::Failure(Transition.Error()); }
    if (WantsQuit_Internal()) { return TResult<bool>::Success(false); }
    auto Objects = m_Scenes.CommitObjects();
    if (!Objects) { return TResult<bool>::Failure(Objects.Error()); }
    if (WantsQuit_Internal()) { return TResult<bool>::Success(false); }
    if (m_pGame)
    {
        auto GameTick = m_pGame->Tick_Internal({m_Input.GetSnapshot(), Time.Value(), &m_Scenes, m_pGame.get(), &m_Audio, 0});
        if (!GameTick) { return TResult<bool>::Failure(GameTick.Error()); }
    }
    if (WantsQuit_Internal()) { return TResult<bool>::Success(false); }
    auto Tick = m_Scenes.Tick(Time.Value(), m_Input.GetSnapshot());
    if (!Tick) { return TResult<bool>::Failure(Tick.Error()); }
    if (WantsQuit_Internal()) { return TResult<bool>::Success(false); }
    auto Audio = m_Audio.Tick();
    if (!Audio) { return TResult<bool>::Failure(Audio.Error()); }
    auto Begin = m_Renderer.BeginFrame(m_Settings.Window.Width, m_Settings.Window.Height, m_Settings.ClearColor);
    if (!Begin) { return TResult<bool>::Failure(Begin.Error()); }
    auto Draw = m_Scenes.Draw(m_Renderer.GetContext());
    if (!Draw) { return TResult<bool>::Failure(Draw.Error()); }
    if (WantsQuit_Internal()) { m_Renderer.CancelFrame(); return TResult<bool>::Success(false); }
    auto Present = m_Renderer.EndFrame();
    if (!Present) { return TResult<bool>::Failure(Present.Error()); }
...
TResult<bool> FApplication::Step(double NowSeconds)
{
    ...
    {
        Shutdown();
        if (Result) { Result = TResult<bool>::Success(false); }
    }
    return Result;
}
void FApplication::Shutdown() noexcept
{
    ...
    TGuardValue Guard(m_bBusy, true);
    m_Scenes.Shutdown();
    if (m_pGame) { m_pGame->Shutdown_Internal(); m_pGame.reset(); }
    m_Audio.Shutdown();
