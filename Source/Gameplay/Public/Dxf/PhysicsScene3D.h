...
{
/**
 * 固定更新で3D物理を駆動するシーン。更新計画は時刻管理が作り、呼び出し元は一つ。
 * 可変更新の入力Snapshotは消費も変更もしない。音声は固定更新で扱わない。
 */
class DPhysicsScene3D : public DGameScene
{
public:
	/**
	 ...
	 */
	explicit DPhysicsScene3D(Toolbox::FFixedStepSettings Settings = {}) : m_Scheduler(Settings)
	{
	}
	/**
	 * 物理ワールドを取得する。
	 */
	FORCEINLINE FPhysicsWorld3D& GetPhysicsWorld() noexcept
	{
		return m_World;
	}
	/**
	 * 物理ワールドを取得する。
	 */
	FORCEINLINE const FPhysicsWorld3D& GetPhysicsWorld() const noexcept
	{
		return m_World;
	}
	/**
