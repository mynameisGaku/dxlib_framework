...
Toolbox::int32 WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, Toolbox::int32)
{
	try
	{
		...
		Settings.Window.Title = "dxlib_framework - Component UI sample (2D / 3D)";
		Settings.Window.Width = 1280;
		Settings.Window.Height = 720;
		// sln配置先を基準にするアセットの起点。開発パス設定がなければexe配置先を使う。
		const Toolbox::FPath ProjectRoot = Dxf::ResolveEntryProjectRoot("UISample.dxfpaths");
		...
		// 処理結果。
		auto Result = Runner.Run(Application,
		                         Dxf::UiSample::MakeTitleScene(Toolbox::MakeShared<Dxf::UiSample::FUiSampleState>()));
		if (!Result)
		{
