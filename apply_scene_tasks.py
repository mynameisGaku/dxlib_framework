#!/usr/bin/env python3
"""Install the pinned Scene Task lifetime update. Dry-run unless --apply is supplied."""
...
TARGETS = frozenset({'Tools/SceneTaskValidation/Main.cpp', 'Source/Runtime/Public/Dxf/Contexts.h', 'Source/Runtime/Private/Dxf/SceneNavigator.cpp', 'Tests/SceneTaskIntegrationTests.cpp', 'Tests/Support/SceneTaskBackend.h', 'Docs/SceneTaskLifetime.md', 'Source/Runtime/Public/Dxf/SceneNavigator.h', 'CMake/SceneTaskIntegrationTests.cmake', 'Source/Runtime/Public/Dxf/Application.h', 'Source/Runtime/Public/Dxf/TaskDispatcher.h', 'Source/Runtime/Private/Dxf/Application.cpp', 'Tools/SceneTaskValidation/CMakeLists.txt'})
...
def build_plan(root: Path, package: Path = HERE) -> list[Change]:
    ...
    for rel in sorted(TARGETS):
        ...
        if has_changes(root, rel, before is not None):
        ...
        plan.append(Change(rel, before, preserve_text_style(data, before), mode))
    # Do not create or keep a mixed header/implementation pair across the two supported renderer variants.
    header = 'Source/Runtime/Public/Dxf/Application.h'
    implementation = 'Source/Runtime/Private/Dxf/Application.cpp'
    if selected_profiles[header] != selected_profiles[implementation]:
        raise ValueError('Application header/implementation renderer profiles do not match')
    return plan


def atomic_write(path: Path, data: bytes, mode: int) -> None:
    """Write a complete sibling temporary file, then replace a single destination."""
    descriptor, temporary = tempfile.mkstemp(prefix='.dxf-scene-', dir=path.parent)
    temp = Path(temporary)
    try:
...
def apply_plan(root: Path, plan: list[Change]) -> Path | None:
    ...
    for change in plan:
        ...
        if has_changes(root, change.relative, change.before is not None):
            raise ValueError(f'Working tree/index changed after validation: {change.relative}')
    backup = Path(tempfile.mkdtemp(prefix=root.name + '.scene-task-backup-', dir=root.parent))
    backup_records = []
    for change in plan:
...
def main() -> int:
    ...
    try:
        ...
        if not plan:
            ...
            return 0
        for change in plan:
            print(('ADD     ' if change.before is None else 'REPLACE ') + change.relative)
        if not args.apply:
