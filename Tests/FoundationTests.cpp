...
TEST("Handle outlives storage without dangling pointer")
{
    ...
    { TSlotMap<DObject> Storage; Handle = Storage.Insert(std::make_unique<DTestObject>()); }
    REQUIRE(!Handle.Get());
}
...
{
    TSlotMap<DObject> Storage;
    for (int Index = 0; Index < 128; ++Index) { Storage.Insert(std::make_unique<DTestObject>()); }
    for (auto Handle : Storage.Snapshot()) { REQUIRE(Storage.Remove(Handle)); }
...
TEST("SceneClock pause and scale preserve unscaled time")
{
    ...
    FFrameTime Time; Time.DeltaSeconds = 0.1; Time.UnscaledDeltaSeconds = 0.1;
...
{
    FInputStateTracker Tracker;
    FRawInput State; State.Keys[static_cast<std::size_t>(EKey::A)] = true; State.MouseButtons[0] = true;
    Tracker.Advance(State);
...
{
    FInputStateTracker Tracker;
    FRawInput State; State.Pads[0].bConnected = true; State.Pads[0].Buttons[0] = true; State.Pads[0].LeftX = 0.8f;
    Tracker.Advance(State);
...
{
    FInputMap Map;
    Map.Bind("Jump", EKey::Space); Map.Bind("Jump", EKey::W);
    FInputStateTracker Tracker;
    FRawInput Raw; Raw.Keys[static_cast<std::size_t>(EKey::Space)] = true;
    Tracker.Advance(Raw); Map.Update(Tracker.GetSnapshot());
    ...
    Tracker.Advance(Raw); Map.Update(Tracker.GetSnapshot());
    REQUIRE(Map.Down("Jump")); REQUIRE(!Map.Released("Jump"));
    Tracker.Advance({}); Map.Update(Tracker.GetSnapshot()); REQUIRE(Map.Released("Jump"));
}
