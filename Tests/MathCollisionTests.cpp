...
TEST("Sphere collisions avoid overflow when finite shapes exceed float distance range")
{
	// 境界箱は有限だが、中心間距離と半径の和が単精度の上限を超える球。
	const FSphere Left{{-1.2e38f, -1.2e38f, -1.2e38f}, 1.8e38f};
	const FSphere Right{{1.2e38f, 1.2e38f, 1.2e38f}, 1.8e38f};
	REQUIRE(IsValid(Left));
	REQUIRE(IsValid(Right));
	REQUIRE(Bounds(Left).Intersects(Bounds(Right)));
	// 中心間距離は約4.157e38、半径の和は3.6e38なので接触していない。
	REQUIRE(!Intersects(Left, Right));
	REQUIRE(!Intersects(Right, Left));
	// 同じ数値範囲でも、実際に重なる二球は検出する。
	REQUIRE(Intersects(Left, FSphere{{0.8e38f, 0.8e38f, 0.8e38f}, 1.8e38f}));
}

TEST("Box sphere face contacts remain symmetric at every surface position")
{
	// 箱の面と球が正確に接する配置を、全種類の箱と両方の引数順で調べる。
	const TVector<FCollisionShape> Boxes{FAABB{{-1, -1, -1}, {1, 1, 1}}, FOBB{{}, {1, 1, 1}}, FCube{{}, 1}};
	for (const auto& Box : Boxes)
	{
		for (int32 Y = 0; Y <= 20; ++Y)
		{
			...
			{
				// X方向に半径だけ離し、球中心を面全体に移動させる。
				const FSphere Sphere{{2, -1 + static_cast<f32>(Y) * 0.1f, -1 + static_cast<f32>(Z) * 0.1f}, 1};
				REQUIRE(Intersects(Box, Sphere));
...
TEST("Box sphere closest distance respects gaps corners and finite large coordinates")
{
	// 面、辺、頂点への接触と、箱の内部にある点を検証する。
	const TVector<FCollisionShape> Boxes{FAABB{{-1, -1, -1}, {1, 1, 1}}, FOBB{{}, {1, 1, 1}}, FCube{{}, 1}};
	for (const auto& Box : Boxes)
	{
		REQUIRE(Intersects(Box, FSphere{{1.6f, 1.8f, 0}, 1}));
		REQUIRE(!Intersects(Box, FSphere{{1.7f, 1.8f, 0}, 1}));
		REQUIRE(Intersects(Box, FSphere{{1.4f, 1.8f, 1.4f}, 1}));
		REQUIRE(!Intersects(Box, FSphere{{1.5f, 1.8f, 1.5f}, 1}));
		REQUIRE(Intersects(Box, FSphere{{}, 0}));
		REQUIRE(Intersects(Box, FSphere{{2.0005f, 0, 0}, 1}, 0.001f));
		REQUIRE(Intersects(FSphere{{2.0005f, 0, 0}, 1}, Box, 0.001f));
		REQUIRE(!Intersects(Box, FSphere{{2.0005f, 0, 0}, 1}, 0.0001f));
	}
	// 境界箱の候補にはなるが、距離の二乗が単精度では無限大になる離れた配置。
	const FAABB LargeBox{{-1e20f, -1e20f, -1e20f}, {1e20f, 1e20f, 1e20f}};
	const FSphere LargeSphere{{2e20f, 2e20f, 2e20f}, 1.5e20f};
	REQUIRE(!Intersects(LargeBox, LargeSphere));
	REQUIRE(!Intersects(LargeSphere, LargeBox));
	REQUIRE(Intersects(LargeBox, FSphere{{2e20f, 2e20f, 2e20f}, 2e20f}));
}

TEST("Rotated box sphere contacts handle rounding allowed by axis validation")
{
	// 正規化と直交の許容誤差を含む軸でも、実際の面までの距離を使う。
	const FOBB Box{{}, {100, 100, 1}, {FVector3{1, 0, 0}, FVector3{0.00009f, 1, 0}, FVector3{0, 0, 1}}};
	REQUIRE(IsValid(Box));
	// 面上の点(100.0045, 50, 0)から、面の外向き法線へ半径0.25だけ離す。
	const FSphere Sphere{{100.2545f, 49.9999775f, 0}, 0.25f};
	REQUIRE(Intersects(Box, Sphere));
	REQUIRE(Intersects(Sphere, Box));
	REQUIRE(!Intersects(Box, FSphere{{100.2555f, 49.9999775f, 0}, 0.25f}));
	// 厚さがない箱も、点と同じ位置で接触する。
	const FOBB Flat{{}, {1, 1, 0}};
	REQUIRE(Intersects(Flat, FSphere{{0, 0, 1}, 1}));
	REQUIRE(!Intersects(Flat, FSphere{{0, 0, 1.001f}, 1}));
}
