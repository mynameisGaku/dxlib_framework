...
public:
	...
	void DeleteTexture(int Handle) noexcept override
	{
		...
		m_Trace.DeletedTextures.push_back(Handle);
		m_Trace.Events.push_back("delete-texture");
	}
	TResult<int> LoadSound(const std::string&, const FSoundLoadOptions&) override
	{
		...
		{
			return TResult<int>::Failure(EErrorCode::NotFound, "missing sound");
		}
		int Handle = m_Trace.NextHandle++;
	...
	void DeleteSound(int Handle) noexcept override
	{
		...
		m_Trace.DeletedSounds.push_back(Handle);
		m_Trace.Events.push_back("delete-sound");
	}
	...
	void DeleteFont(int Handle) noexcept override
	{
		...
		m_Trace.DeletedFonts.push_back(Handle);
	}
	...
	{
		m_Trace.Events.push_back("clear");
		return {};
	}
	TResult<void> ResetState(int, int) override
	{
		++m_Trace.Resets;
		return {};
	}
	...
	TResult<void> Present() override
	{
		...
		m_Trace.Events.push_back("present");
		return {};
	}
	TResult<void> Initialize(const FWindowSettings&) override
	{
		m_Trace.Events.push_back("init");
		return m_Trace.bFailPlatform ? TResult<void>::Failure(EErrorCode::BackendFailure, "init failed") : TResult<void>{};
	}
	void Shutdown() noexcept override
	{
		m_Trace.Events.push_back("shutdown");
	}
