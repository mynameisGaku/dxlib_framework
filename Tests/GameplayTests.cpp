...
protected:
	TResult<void> OnInitialize(const FInitContext&) override
	{
		...
		return m_pCounters->bFailInitialize ? TResult<void>::Failure(EErrorCode::InitializationFailed, "component init") : TResult<void>{};
	}
	...
	void OnDeinitialize() noexcept override
	{
		...
		m_pCounters->Order.push_back("component-stop");
	}
private:
...
protected:
	TResult<void> OnInitialize(const FInitContext&) override
	{
		...
		return m_pCounters->bFailInitialize ? TResult<void>::Failure(EErrorCode::InitializationFailed, "object init") : TResult<void>{};
	}
	void OnTick(const FTickContext&) override
	{
		++m_pCounters->Tick;
	...
	{
		++m_pCounters->Deinitialize;
		m_pCounters->Order.push_back("object-stop");
		if (m_Stop)
		{
			m_Stop();
		}
...
protected:
	...
	{
		++m_pCounters->Initialize;
		return m_pCounters->bFailInitialize ? TResult<void>::Failure(EErrorCode::InitializationFailed, "scene init") : TResult<void>{};
	}
	void OnTick(const FTickContext&) override
	{
		++m_pCounters->Tick;
...
TEST("Self destruction in tick skips owned components and later draw")
{
	...
	Self = Objects.Spawn<DCountingObject>(A, [&]
	{
		Self.Get()->Destroy();
	}).Value();
...
TEST("Shutdown is idempotent and stops components before their owner")
{
	...
	Objects.Shutdown_Internal();
	REQUIRE(Counts.Order == std::vector<std::string>({"component-stop", "object-stop"}));
...
TEST("Mutations submitted from shutdown wait for the next boundary")
{
	...
	FCounters B;
	FGameObjectCollection Objects;
	auto Handle = Objects.Spawn<DCountingObject>(A, std::function<void()>{}, std::function<void()>{}, [&]
	{
...
TEST("Children added to an existing object during another initializer wait for a new boundary")
{
	...
	Objects.FreezeBoundary_Internal();
	REQUIRE(Objects.CommitBoundary_Internal(World.GetInit()));
	REQUIRE(Objects.Spawn<DCountingObject>(B, std::function<void()>{}, [&]
	{
...
TEST("Scene override cannot bypass object dispatch")
{
	...
	auto Scene = std::make_unique<DCountingScene>(SceneCounts);
	REQUIRE(Scene->Spawn<DCountingObject>(ObjectCounts));
	REQUIRE(World.GetScenes().RequestChange(std::move(Scene)));
...
TEST("Scene switch preserves the active scene until commit")
{
	...
	REQUIRE(World.GetScenes().RequestChange(std::make_unique<DCountingScene>(B)));
...
TEST("A request made inside OnEnter survives for the next scene boundary")
{
	...
	FCounters A;
	FCounters B;
	auto Scene = std::make_unique<DCountingScene>(A, [&]
	{
		(void)World.GetScenes().RequestChange(std::make_unique<DCountingScene>(B));
	});
	REQUIRE(World.GetScenes().RequestChange(std::move(Scene)));
	REQUIRE(World.GetScenes().Commit());
	REQUIRE(B.Initialize == 0);
...
TEST("Handles from a destroyed scene cannot resolve into the next scene")
{
	...
	FCounters SceneCounts;
	FCounters Counts;
	auto First = std::make_unique<DCountingScene>(SceneCounts);
	auto Old = First->Spawn<DCountingObject>(Counts).Value();
	REQUIRE(World.GetScenes().RequestChange(std::move(First)));
	REQUIRE(World.GetScenes().Commit());
	REQUIRE(Old.Get());
	auto Second = std::make_unique<DCountingScene>(SceneCounts);
	auto New = Second->Spawn<DCountingObject>(Counts).Value();
	REQUIRE(World.GetScenes().RequestChange(std::move(Second)));
	REQUIRE(World.GetScenes().Commit());
	REQUIRE(!Old.Get());
	REQUIRE(New.Get());
	World.GetScenes().Shutdown();
...
TEST("GameObject initialization exception is converted into an error and cleaned up")
{
	...
	FCounters Counts;
	FGameObjectCollection Objects;
	auto Handle = Objects.Spawn<DCountingObject>(Counts, std::function<void()>{}, []
	{
